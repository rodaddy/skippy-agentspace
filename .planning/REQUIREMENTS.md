...
- [ ] **CMD-01**: `/skippy:reconcile` works end-to-end against a real `.planning/` project with completed GSD phases -- reads PLAN.md, compares to execution output, reports deviations
- [ ] **CMD-02**: `/skippy:update` hardened -- clones to `~/.cache/` (not `/tmp/`), parses `.versions` safely (no `source`), uses full SHA hashes, survives macOS reboot
