...
**Success Criteria** (what must be TRUE):
  1. Four new reference docs exist under `skills/skippy/references/`: `phased-execution.md`, `state-tracking.md`, `plan-structure.md`, `checkpoints.md` (wave-parallelism folded into phased-execution.md per user decision)
...
**Success Criteria** (what must be TRUE):
  1. `skills/skippy/commands/review.md` exists following the standard command pattern (YAML frontmatter + process)
...
**Success Criteria** (what must be TRUE):
  ...
  4. `INDEX.md` regenerated with `/skippy:review` in skippy's command list
