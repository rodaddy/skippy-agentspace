...
/**
 * generate-ulart-image - Image Generation CLI
 *
 ...
 * Also supports GEMINI_API_KEY as fallback to GOOGLE_API_KEY
 */
...
const DEFAULTS = {
  ...
  size: "16:9" as Size,
  output: `${process.env.HOME}/Downloads/pai-generated-${Date.now()}.png`,
};
...
OPTIONS:
  ...
  --output <path>            Output file path (default: ~/Downloads/pai-generated-<timestamp>.png)
...
EXAMPLES:
  ...
  generate-ulart-image --model nano-banana-pro --prompt "Abstract editorial illustration..." --size 2K --aspect-ratio 16:9
  ...
  generate-ulart-image --model nano-banana --prompt "Abstract editorial illustration..." --size 16:9
...
async function generateWithFlux(prompt: string, size: ReplicateSize, output: string): Promise<void> {
  ...
  const response = await fetch(result as unknown as string);
...
async function generateWithNanoBananaPro(
  ...
  // Support both GOOGLE_API_KEY and GEMINI_API_KEY (from ~/.zshrc)
  const apiKey = process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new CLIError("Missing environment variable: GOOGLE_API_KEY or GEMINI_API_KEY");
  }
  ...
  const response = await ai.models.generateContent({
    model: process.env.GEMINI_IMAGE_MODEL || "image",
    contents: [{ parts }],
    config: {
