...
### $env - Environment Variables (PAID ONLY)

**CRITICAL: $env is a PAID feature.** It does NOT work on n8n Community Edition.
`$env.VAR` in expressions and `N8N_AVAILABLE_ENV_VARS` are completely ignored.
Never use $env in workflow expressions on Community Edition. Hardcode values
or pass through $json from Code nodes instead.

```javascript
// PAID EDITION ONLY -- do not use on Community Edition
{{$env.API_KEY}}
{{$env.DATABASE_URL}}
```
